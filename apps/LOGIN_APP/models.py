...
class Manager(models.Manager):
    def validator(self, postData):
        ...
        if len(postData['password']) < 8:
            ...
            errors['password'] = "password must contain a integer"
        elif re.search('[A-Z]',password) is None: 
            errors['password'] = "password must contain an UPPERCASE"
        ...
        if not emailFormat.match(postData['email']): #CHECK FOR EMAIL FORMAT
            errors['emailmatch'] = 'Email must be a valid email'
        ...
        #     errors['passlen'] = 'Passwords must be at least 8 characters long'
